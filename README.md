...
（当前只跟踪 Ltd Edition、Taneda；其中放票时间已核实、可推送者 = Ltd Edition。
Taneda 为估计时间，标「时间待确认」、不自动推送）。

## 加城市 / 改规则
只动 `pipeline/fetch_reservations.py` 的 `RESTAURANTS` 表。当前为 Ltd Edition、Taneda 两家。
